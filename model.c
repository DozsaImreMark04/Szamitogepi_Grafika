...
void draw_model(Model* model) {
    int has_tex = (model->texture_id != 0 && model->texcoord_count > 0);
    ...
    for (int i = 0; i < model->face_count; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            int ti = model->faces[i].t[j] - 1;

            if (model->vertices[vi].y > 2.8f) {
                glColor3f(0.10f, 0.65f, 0.12f);
            } else {
                glColor3f(0.40f, 0.24f, 0.08f);
            }

            if (has_tex && ti >= 0 && ti < model->texcoord_count) {
                glTexCoord2f(model->texcoords[ti].u, model->texcoords[ti].v);
            }

            if (ni >= 0 && ni < model->normal_count) {
